...
class MCPClient {
    ...
    async connectToServer(serverScriptPath) {
        ...
        this.client = new Client({ name: "mcp-weather-client", version: "0.2.0" }, {
            capabilities: {
                tools: {},
            },
        });
    ...
    async processQuery(messages) {
        ...
        while (true) {
            ...
            for (const tool of tool_calls) {
                ...
                console.log(chalk.cyan(`[Calling tool ${name} with args ${JSON.stringify($arguments)}]`));
                const toolResponse = await this.client.request({
                    method: "tools/call",
                    params: { name, arguments: $arguments },
                }, CallToolResultSchema);
                console.log(chalk.cyan(`[Tool response ${JSON.stringify(toolResponse)}]`));
...
async function main() {
    if (process.argv.length < 3) {
        console.log(chalk.red(`Usage: ts-node client.ts <path_to_server_script>`));
        process.exit(1);
    }
